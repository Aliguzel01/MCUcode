...
void GPIO_IRQInterruptConfig(uint8_t IRQNumber, uint8_t EnorDi)
{
   if(EnorDi==ENABLE)
   {
	   if(IRQNumber <= 31)
	   {
		   //ISER0 KAYDINI PROGRAMLA
		    *NVIC_ISER0 |=( 1 << IRQNumber);

	   }else if(IRQNumber > 31 && IRQNumber < 64 )
	   {
		   //ISER1 KAYDINI PROGRAMLA
		   *NVIC_ISER1 |=( 1 << (IRQNumber % 32) );
	   }else if(IRQNumber >=64 && IRQNumber < 96 )
	   {
		   //ISER2 KAYDINI PROGRAMLA
   ...
   }else
   {
	   if(IRQNumber <=31)
	   {
		   //ICER0 KAYDINI PROGRAMLA
		    *NVIC_ICER0 |=( 1 << IRQNumber);

	   }else if(IRQNumber > 31 && IRQNumber < 64 )
	   {
		   //ICER1 KAYDINI PROGRAMLA
		    *NVIC_ICER1 |=( 1 << (IRQNumber % 32) );

	   }else if(IRQNumber >=64 && IRQNumber <96 )
	   {
		   //ICER2 KAYDINI PROGRAMLA
...
{
//pin numarasına karşılık gelen EXTI PR yazmacını temizle
